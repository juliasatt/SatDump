...
namespace uhrit
{
    ...
    UHRITProductizer::UHRITProductizer(std::string instrument_id, bool sweep_x, std::string cache_path)
        : should_sweep_x(sweep_x), instrument_id(instrument_id), compo_cache_path(cache_path)
    {
        ...
        if (can_make_composites)
            compositeGeneratorThread = std::thread(&UHRITProductizer::compositeThreadFunc, this);
    }

    UHRITProductizer::~UHRITProductizer()
    {
        if (can_make_composites)
        {
            int queue_size;
    ...
    void UHRITProductizer::compositeThreadFunc()
    {
    ...
    void UHRITProductizer::saveImage(image::Image img,
                                    int bit_depth,
                                    ...
                                    time_t timestamp,
                                    std::string region,
                                    uhrit::ImageNavigationRecord *image_navigation_record,
